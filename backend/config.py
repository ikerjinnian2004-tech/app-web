...
class Settings(BaseSettings):
    ...
    exam_duration_minutes: int = Field(default=90, alias="EXAM_DURATION_MINUTES")
    database_pool_size: int = Field(default=10, alias="DATABASE_POOL_SIZE")
    database_max_overflow: int = Field(default=20, alias="DATABASE_MAX_OVERFLOW")
    database_pool_timeout_seconds: int = Field(
        default=30, alias="DATABASE_POOL_TIMEOUT_SECONDS"
    )
    database_pool_recycle_seconds: int = Field(
        default=1800, alias="DATABASE_POOL_RECYCLE_SECONDS"
    )

    sandbox_timeout_seconds: int = Field(default=3, alias="SANDBOX_TIMEOUT_SECONDS")
    sandbox_max_output_chars: int = Field(
        default=5000, alias="SANDBOX_MAX_OUTPUT_CHARS"
    )
    ...
    sandbox_mem_limit_mb: int = Field(default=64, alias="SANDBOX_MEM_LIMIT_MB")
    sandbox_cpu: float = Field(default=0.5, alias="SANDBOX_CPU")
    sandbox_pids_limit: int = Field(default=50, alias="SANDBOX_PIDS_LIMIT")
    evidencia_max_bytes: int = Field(default=15_000_000, alias="EVIDENCIA_MAX_BYTES")
    evidencia_duracion_segundos: int = Field(
        default=15, alias="EVIDENCIA_DURACION_SEGUNDOS"
    )

    allowed_origins: Annotated[list[str], NoDecode] = Field(alias="ALLOWED_ORIGINS")
    log_level: str = Field(default="INFO", alias="LOG_LEVEL")
    healthcheck_interval_seconds: int = Field(
        default=30, alias="HEALTHCHECK_INTERVAL_SECONDS"
    )
